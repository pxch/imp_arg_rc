...
class DependencyGraph(object):
    ...
    def build(self, deps: List[Dependency]):
    ...
    def get_parent(self, token_idx, msg_prefix=''):
        ...
        if len(parent) == 0:
            return 'root', -1
        if len(parent) > 1 or len(list(parent.values())[0]) > 1:
            log.warning(
                '{}: In sentence #{}, token #{} has more than 1 non-extra '
                'head token: {}'.format(
                    msg_prefix, self._sent_idx, token_idx, parent))
        parent_label, parent_indices = list(parent.items())[0]
        return parent_label, parent_indices[0]
